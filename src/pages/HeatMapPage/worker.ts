...
const fatalAccidents = () => {
  return featureCollection(
    accidents.features.filter((value) => {
...
onmessage = (message) => {
  ...
  const bycicleRentalsWithRadius = bycicles.features.map((el) =>
    circle(el, message.data[0], options),
  );
  const rentalsBBox = bycicleRentalsWithRadius.map(bbox);
  const collections = rentalsBBox.map((el) =>
    pointGrid(el, message.data[0] / 8, { units: "meters" }),
  );
  const myList = [];
  ...
  if (message.data[2]) {
    const accidentsWithRadius = fatalAccidents().features.map((el) =>
      circle(el, message.data[1], options),
    );
    const pointsToRemove = pointsWithinPolygon(
      allPoints,
      featureCollection(accidentsWithRadius),
    );
    allPoints.features = allPoints.features.filter(
      (point) => !pointsToRemove.features.includes(point),
    );
  }
